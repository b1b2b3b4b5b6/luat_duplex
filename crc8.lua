require "utils"
require "pm"
crc8 = {}
local crc8_table = {
     
};

function crc8.get(data)
    local crc8 = 0
    --log.info("crc8 len: ", string.len(data))
    for p = 1, string.len(data), 1 do
        --log.info("crc temp ", string.format( "%x",crc8))
        crc8 = crc8_table[bit.bxor(crc8, string.byte(data, p)) + 1]
    end
    return crc8
end


